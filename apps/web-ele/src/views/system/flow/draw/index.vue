...
const CustomTranslateModule = {
  translate: [
    ...
    function (template: string, replacements?: Record<string, string>) {
      ...
      const translated =
        combinedZhTranslations[
          template as keyof typeof combinedZhTranslations
        ] || template;
      return translated.replaceAll(/{([^}]+)}/g, (_: string, key: string) => {
...
const modeler = ref<InstanceType<typeof BpmnModeler>>();
...
const initModeler = () => {
  ...
  modeler.value = new BpmnModeler({
    ...
    // 添加中文翻译
    i18n: {
      translate(template: string, replacements?: Record<string, string>) {
        replacements = replacements || {};
        const translated =
          combinedZhTranslations[
            template as keyof typeof combinedZhTranslations
...
const saveModel = async () => {
  ...
  try {
    ...
    if (processElements.length > 0) {
      ...
      processName =
        processElement.businessObject.name || processElement.name || processId;
...
const handleOpenProcess = () => {
  const fileInput = document.querySelector('#bpmn-upload') as HTMLInputElement;
  if (fileInput) {
    fileInput.click();
  }
};
...
<template>
  <Page auto-content-height class="flex flex-col h-full">
    <div
      class="flex justify-between items-center p-4 bg-white border-b border-gray-200 flex-shrink-0"
    >
      ...
      <div class="flex gap-2">
        ...
        <ElButton @click="createNewProcess"> 新建 </ElButton>
        <input
          type="file"
          accept=".bpmn"
          ...
          id="bpmn-upload"
        />
        <ElButton @click="handleOpenProcess"> 打开 </ElButton>
        <ElButton @click="handleCopyXML"> 复制XML </ElButton>
        <ElButton type="primary" @click="saveModel"> 保存 </ElButton>
        <ElButton type="danger" @click="exportAsImage"> 导出图片 </ElButton>
      </div>
    </div>
    <div
      class="flex flex-1 gap-4"
      style="height: calc(var(--vben-content-height) - 100px)"
    >
      <div
        ref="containerRef"
        class="flex-1 bg-white border border-gray-300 rounded-md overflow-hidden relative"
      >
        <!-- 缩放控制栏 -->
        <div class="absolute bottom-20 right-8 flex flex-col gap-1 z-100">
          <ElButton
            ...
            @click="zoomIn"
            title="放大"
            class="w-8 h-8 flex items-center justify-center p-0 rounded"
          >
            +
          </ElButton>
          <ElButton
            ...
            @click="zoomReset"
            title="重置"
            class="w-8 h-8 flex items-center justify-center p-0 rounded text-xs"
          >
            100%
          </ElButton>
          <ElButton
      ...
      <div
        ...
        class="w-[300px] bg-white border border-gray-300 rounded-md overflow-auto flex-shrink-0"
      ></div>
    </div>
  </Page>
</template>
